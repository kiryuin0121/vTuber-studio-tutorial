...
const VRMAvatar = ({ avatar, ...props }) => {
    ...
    (results) => {
      ...
      if (results.faceLandmarks) {
        riggedFace.current = Face.solve(results.faceLandmarks, {
          runtime: "mediapipe", // `mediapipe` または `tfjs`
          video: videoElement,
          imageSize: { width: 640, height: 480 }, //mediapipe/camera_utilsのサイズに合わせる
          smoothBlink: false, // 左右のまばたきの遅延を滑らかにします
          blinkSettings: [0.25, 0.75], // まばたきの感度の上限と下限を調整します
        });
      }
      // 体
  ...
  useFrame((_, delta) => {
    ...
    // カメラが動作していない場合はLevaGUIによって表情アニメーションを制御する
    if (!videoElement) {
      [
        {
          name: "aa",
